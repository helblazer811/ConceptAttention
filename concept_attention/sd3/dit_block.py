    ...
    def __call__(
        ...
        **kwargs,
        ...
        if encoder_hidden_states is not None:
            ...
            return hidden_states, cross_attention_maps


@maybe_allow_in_graph
class CustomJointTransformerBlock(nn.Module):
    r"""
    ...
    def forward(
        ...
        if self.context_pre_only:
            ...
            norm_concept_hidden_states = self.norm1_context(concept_hidden_states, temb)
        else:
            norm_encoder_hidden_states, c_gate_msa, c_shift_mlp, c_scale_mlp, c_gate_mlp = self.norm1_context(
                encoder_hidden_states, emb=temb
            )
            norm_concept_hidden_states, c_gate_msa2, c_shift_mlp2, c_scale_mlp2, c_gate_mlp2 = self.norm1_context(
        ...
        concept_attention_outputs = {
            ...
            # "cross_attention_maps": cross_attention_maps,
            # "output_space_maps": output_space_maps.detach().cpu(),
            # "concept_attention_maps": concept_attention_maps.detach().cpu(),
        }

        # # Do an output space projection of the prompt tokens
        # output_space_maps = einops.einsum(
        #     attn_output,
        #     context_attn_output,
        #     "batch patches dim, batch concepts dim -> batch concepts patches",
        # )
        ...
        if self.context_pre_only:
            ...
            # Process the concept attention outputs
            concept_attn_output = c_gate_msa2.unsqueeze(1) * concept_attn_output
            ...
            norm_concept_hidden_states = norm_concept_hidden_states * (1 + c_scale_mlp2[:, None]) + c_shift_mlp2[:, None]
            if self._chunk_size is not None:
                # "feed_forward_chunk_size" can be used to save memory
                concept_ff_output = _chunked_feed_forward(
                    self.ff_context, norm_concept_hidden_states, self._chunk_dim, self._chunk_size
                )
            else:
                concept_ff_output = self.ff_context(norm_concept_hidden_states)
            concept_hidden_states = concept_hidden_states + c_gate_mlp2.unsqueeze(1) * concept_ff_output


        return encoder_hidden_states, hidden_states, concept_hidden_states, concept_attention_outputs
...
class CustomSD3Transformer2DModel(
    ...
    def forward(
        ...
        hidden_states: torch.FloatTensor,
        encoder_hidden_states: torch.FloatTensor = None,
        concept_hidden_states: torch.FloatTensor = None,
        ...
        # Make array to store the cross attention maps
        ...
        for index_block, block in enumerate(self.transformer_blocks):
            ...
            if torch.is_grad_enabled() and self.gradient_checkpointing and not is_skip:
                ...
                encoder_hidden_states, hidden_states, concept_hidden_states, layer_concept_attention_outputs = block(
        ...
        if not return_dict:
            return (output, concept_attention_outputs)

        return Transformer2DModelOutput(sample=output), concept_attention_outputs